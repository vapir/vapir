module Watir
  module Specifier
    ...
    LocateAliases=Hash.new{|hash,key| []}.merge!({ :text => [:textContent],
                                                     :class => [:className],
          ...
          specifier.all? do |(how, what)|
            if how==:types
              what.any? do |type|
                ...
                candidate_attributes << candidate.getAttribute(:type) if candidate.hasAttribute(:type) # this won't work for default types, like a plain <input> where type defaults to text
