...
class TC_Radios < Test::Unit::TestCase
    ...
    def test_Radio_Exists
       ...
       assert_nil(browser.radio(:name, "missingname"))   
       assert_nil(browser.radio(:id, "missingid"))   
    end
   ...
   def test_onClick
       ...
       assert_false(browser.radio!(:name, "box5").checked?)
       assert_false(browser.button!(:value , "foo").enabled?)
    ...
    def test_radio_iterators
        ...
        browser.radios.each do |r|
            ...
            assert_equal( browser.radio!(:index, index).checked? , r.checked? )
            index+=1
        end
