...
class TC_NewWindow < Test::Unit::TestCase
    ...
    def test_attach_to_new_window_with_same_title_as_parent
        ...
        ff_new = browser.class.attach(:title, 'New Browser Launcher')
        assert(ff_new.text.include?('PASS'))
        ff_new.close
    end
     
    def test_new_window_exists
        assert_raises(NoMatchingWindowFoundException , "NoMatchingWindowFoundException was supposed to be thrown") { browser.class.attach(:title, "missing_title") }
        assert_raises(NoMatchingWindowFoundException , "NoMatchingWindowFoundException was supposed to be thrown") { browser.class.attach(:url, "missing_url") }
    end
end
