...
class TC_Links_Multi < Test::Unit::TestCase
  ...
  def test_not_found_single
    ...
    assert_match(/^Unable to locate Watir::.*Link, using attributes: \{:id=>"Missing"\}$/, exception.message)
  end
  ...
  def test_not_found_with_multi
    ...
    assert_match(/^Unable to locate Watir::.*Link, using attributes: \{:class=>"Missing"\}, index 2$/, exception.message)
  end
